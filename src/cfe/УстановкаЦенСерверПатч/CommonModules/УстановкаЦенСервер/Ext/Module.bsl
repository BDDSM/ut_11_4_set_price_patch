...
Процедура уст_ц_ИнициализироватьВыбранныеЦеныНоменклатуры(Форма, МассивВидовЦен = Неопределено)
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		Если СтрокаТЧ.Статус = Перечисления.СтатусыДействияВидовЦен.Действует Или
			...
			//-- malikov_pro
			Если НЕ ТипЗнч(Форма) = Тип("Структура") Тогда
				//-- malikov_pro
			...
			Иначе
				...
				уст_ц_УстановкаЦенСервер.ЗаполнитьТаблицуЗначенийИзТаблицыЗначений(НоваяСтрока.ПорогиСрабатывания, СтрокаТЧ.ПорогиСрабатывания, "ПорогиСрабатывания");
				уст_ц_УстановкаЦенСервер.ЗаполнитьТаблицуЗначенийИзТаблицыЗначений(НоваяСтрока.ПравилаОкругленияЦены, СтрокаТЧ.ПравилаОкругленияЦены, "ПравилаОкругленияЦены");
				уст_ц_УстановкаЦенСервер.ЗаполнитьТаблицуЗначенийИзТаблицыЗначений(НоваяСтрока.ЦеновыеГруппы, СтрокаТЧ.ЦеновыеГруппы, "ЦеновыеГруппы");
				уст_ц_УстановкаЦенСервер.ЗаполнитьТаблицуЗначенийИзТаблицыЗначений(НоваяСтрока.ВлияющиеВидыЦен, СтрокаТЧ.ВлияющиеВидыЦен, "ВлияющиеВидыЦен");
				НоваяСтрока.ВлияющиеЦены = СтрокаТЧ.ВлияющиеЦены;
				НоваяСтрока.ЗависимыеЦены = СтрокаТЧ.ЗависимыеЦены;
...
Процедура уст_ц_ПостроитьДеревоЦен(Форма, ИмяТаблицы = "ДеревоЦен") Экспорт
	...
	Иначе
		...
		РеквизитыТаблицыЦен = Форма[ИмяТаблицы].Колонки;
	...
	Если ТипЗнч(Форма) = Тип("Структура") Тогда
		...
		Для Каждого Колонка Из ДобавляемыеКолонкиТаблицыЦен Цикл
			Форма.ДеревоЦен.Колонки.Добавить(Колонка.Имя, Колонка.ТипЗначения);
		КонецЦикла;
		
		Для каждого Колонка Из УдаляемыеКолонкиТаблицыЦен Цикл
			
			Для каждого СтрК Из Форма.ДеревоЦен.Колонки Цикл
				
				Если СтрК.Имя = Колонка.Имя Тогда
					Форма.ДеревоЦен.Колонки.Удалить(СтрК);	
				КонецЕсли;
...
Процедура уст_ц_ЗагрузитьТоварыВДеревоЦен(Форма, ВыборкаНоменклатура, КэшДанных)
	...
	Иначе
		...
		Если НЕ ТипЗнч(Форма) = Тип("Структура") Тогда
			...
			//-- malikov_pro
		Иначе
			ЭлементыДереваЦен = Форма.ДеревоЦен.Строки;
		КонецЕсли;
		//-- malikov_pro
...
Процедура уст_ц_ПоместитьЦеныВТабличнуюЧасть(Параметры) Экспорт
	...
	//-- malikov_pro
	Если НЕ ТипЗнч(Параметры.Форма) = Тип("Структура") Тогда
		//-- malikov_pro
	...
	Иначе
		...
		ЭлементыНоменклатура = Параметры.Форма.ДеревоЦен.Строки;
	...
	Для Каждого ТекЭлементНоменклатура Из ЭлементыНоменклатура Цикл
		...
		Если НЕ ТипЗнч(Параметры.Форма) = Тип("Структура") Тогда
			...
			//-- malikov_pro	
		Иначе
			ЭлементыХарактеристики = ТекЭлементНоменклатура.Строки;
	...
	Для Каждого ВидЦены Из Параметры.Форма.ВыбранныеЦены Цикл
		...
		Если ВидЦены.Выбрана Или Параметры.СохранятьБазовые И ВидЦены.Влияет Тогда
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(ВременнаяСтруктура, СтрокаЦен, ИмяКолонки); 
				ЕстьКолонкаЦены = ВременнаяСтруктура[ИмяКолонки] <> УИД;
